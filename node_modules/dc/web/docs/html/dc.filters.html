...
<head>
	...
	<title>dc.js Namespace: filters</title>
...
<article>
    <div class="container-overview">
            ...
            <div class="description"><p>The dc.js filters are functions which are passed into crossfilter to chose which records will be<br>accumulated to produce values for the charts.  In the crossfilter model, any filters applied on one<br>dimension will affect all the other dimensions but not that one.  dc always applies a filter<br>function to the dimension; the function combines multiple filters and if any of them accept a<br>record, it is filtered in.</p>
<p>These filter constructors are used as appropriate by the various charts to implement brushing.  We<br>mention below which chart uses which filter.  In some cases, many instances of a filter will be added.</p>
...
<p>Currently these filter objects are also arrays, but this is not a requirement. Custom filters<br>can be used as long as they have the properties above.</p></div>
		...
		<ul class="dummy">
			<li>
				<a href="filters.js.html">filters.js</a>,
				 <a href="filters.js.html#sunlight-1-line-1">line 1</a>
			</li>
		</ul>
	</dd>
